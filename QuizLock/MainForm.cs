...
namespace QuizLock
{
    public partial class MainForm : Form
    {
        ...
        private readonly RadioButton _radLocalExcel = new();
        private readonly RadioButton _radOneDriveExcel = new();
        ...
        private readonly TextBox _txtWorkbookLink = new();
        ...
        private string _workbookLink = string.Empty;
        private GraphExcelClient? _graphClient;
        ...
        private void BuildSetupPanel()
        {
            ...
            _chkCaptureScore.Text = "Save name + score from the quiz results page";
            _chkCaptureScore.Location = new Point(20, 260);
            _chkCaptureScore.Width = 430;
            _chkCaptureScore.Height = 20;
            _chkCaptureScore.CheckedChanged += (_, _) => UpdateResultsFieldsEnabled();
            ...
            var lblDestination = new Label { Text = "Save results to:", AutoSize = true, Location = new Point(20, 342) };

            _radLocalExcel.Text = "A local Excel file (.xlsx)";
            _radLocalExcel.Location = new Point(20, 364);
            _radLocalExcel.Width = 420;
            _radLocalExcel.CheckedChanged += (_, _) => UpdateResultsFieldsEnabled();

            var lblOutputPath = new Label { Text = "Local file:", AutoSize = true, Location = new Point(40, 390) };
            _txtOutputPath.Location = new Point(40, 412);
            _txtOutputPath.Width = 320;
            ...
            _btnBrowseOutput.Location = new Point(365, 410);
            _btnBrowseOutput.Width = 85;
            _btnBrowseOutput.Height = 26;
            _btnBrowseOutput.Click += BtnBrowseOutput_Click;

            _radOneDriveExcel.Text = "A Microsoft OneDrive / Excel Online workbook (writes automatically)";
            _radOneDriveExcel.Location = new Point(20, 444);
            _radOneDriveExcel.Width = 430;
            _radOneDriveExcel.Height = 34;
            _radOneDriveExcel.Checked = true;
            _radOneDriveExcel.CheckedChanged += (_, _) => UpdateResultsFieldsEnabled();

            var lblClientId = new Label { Text = "Azure app Client ID (see README for how to get one):", AutoSize = true, Location = new Point(40, 480) };
            _txtClientId.Location = new Point(40, 502);
            _txtClientId.Width = 410;

            var lblWorkbookLink = new Label { Text = "OneDrive / Excel Online workbook link:", AutoSize = true, Location = new Point(40, 532) };
            _txtWorkbookLink.Location = new Point(40, 554);
            _txtWorkbookLink.Width = 410;
            _txtWorkbookLink.Text = "https://excel.cloud.microsoft/open/onedrive/?docId=F2B0642FA53AA1C8%21s69a0137a6b264c54be7f9ce4663b49c4&driveId=F2B0642FA53AA1C8";

            _btnStart.Text = "Start Lockdown";
            _btnStart.Location = new Point(20, 600);
            _btnStart.Width = 200;
            _btnStart.Height = 35;
            ...
            _setupPanel.Controls.AddRange(new Control[]
            {
                ...
                lblDestination, _radLocalExcel, lblOutputPath, _txtOutputPath, _btnBrowseOutput,
                _radOneDriveExcel, lblClientId, _txtClientId, lblWorkbookLink, _txtWorkbookLink,
                _btnStart, _lblStatus
            });

            UpdateResultsFieldsEnabled();
        }

        private void UpdateResultsFieldsEnabled()
        {
            bool on = _chkCaptureScore.Checked;
            _txtScoreSelector.Enabled = on;
            _radLocalExcel.Enabled = on;
            _radOneDriveExcel.Enabled = on;

            bool local = on && _radLocalExcel.Checked;
            _txtOutputPath.Enabled = local;
            _btnBrowseOutput.Enabled = local;

            bool oneDrive = on && _radOneDriveExcel.Checked;
            _txtClientId.Enabled = oneDrive;
            _txtWorkbookLink.Enabled = oneDrive;
        }
        ...
        private void BtnStart_Click(object? sender, EventArgs e)
        {
            ...
            _useOneDrive = _radOneDriveExcel.Checked;
            _outputPath = _txtOutputPath.Text.Trim();
            _clientId = _txtClientId.Text.Trim();
            _workbookLink = _txtWorkbookLink.Text.Trim();
            _scoreCapturedThisSession = false;

            if (_captureScore)
            {
                if (string.IsNullOrWhiteSpace(_scoreSelector))
                {
                    MessageBox.Show(this, "Fill in the score selector, or uncheck the results-saving option.",
                        "QuizLock", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                if (_useOneDrive && (string.IsNullOrWhiteSpace(_clientId) || string.IsNullOrWhiteSpace(_workbookLink)))
                {
                    MessageBox.Show(this, "Fill in the Azure app Client ID and the workbook link.",
                        "QuizLock", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                if (!_useOneDrive && string.IsNullOrWhiteSpace(_outputPath))
                {
                    MessageBox.Show(this, "Choose a local file to save results to.",
                        "QuizLock", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
        ...
        {
            if (_captureScore && _useOneDrive)
            {
                _btnStart.Enabled = false;
                try
                {
                    _lblStatus.ForeColor = Color.DimGray;
                    _lblStatus.Text = "Signing in to Microsoft (a sign-in window may open)...";
                    _lblStatus.Refresh();

                    _graphClient = new GraphExcelClient(_clientId);
                    await _graphClient.SignInAsync();

                    _lblStatus.Text = "Connecting to your Excel workbook...";
                    _lblStatus.Refresh();
                    await _graphClient.ResolveWorkbookAsync(_workbookLink);
                }
                catch (Exception ex)
                {
                    MessageBox.Show(this,
                        "Couldn't connect to Microsoft / the OneDrive workbook:\n" + ex.Message,
        ...
        private void CoreWebView2_WebMessageReceived(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
        {
            ...
            var name = _quizTakerName;
            var score = result.Score.Trim();
            if (_useOneDrive)
            {
                _ = SaveResultToOneDriveAsync(name, score);
            }
            else
            {
                SaveResultToExcel(name, score);
            }
        }

        private async Task SaveResultToOneDriveAsync(string name, string score)
        {
            try
            {
                ...
                await _graphClient.AppendRowAsync(name, score);
