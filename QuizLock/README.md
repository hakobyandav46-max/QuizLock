...
8. Right before it unlocks, a small dialog shows whatever score it could
   auto-detect from the quiz page (best-effort - see caveat below). Confirm
   or correct it, then it's appended as a new row to the Excel file you set
   in step 6, along with the name, quiz link, and timestamp. You can also
   click "Skip" to not log that attempt at all.
...
instead of text. That's exactly why the score is always shown to you for
confirmation before it's saved - nothing is ever logged silently.
