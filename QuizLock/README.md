...
1. Run `QuizLock.exe`.
2. Paste up to **three** Microsoft Forms links (e.g. `https://forms.office.com/r/...`)
   - only the first is required, the other two are optional. Each time you
     click Start, one of the filled-in links is picked at random - handy for
     giving different quiz-takers different versions to reduce copying
     between them.
